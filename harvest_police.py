#!/usr/bin/env python3
"""
harvest_police.py — the police wire: what the police are obliged to do, what
they actually do, what they can see, and what you may refuse.

After the Law Enforcement section of Suppression: the duty to protect and its
absence, immunity and the code of silence, corruption and extortion, criminal
infiltration of the force, use of force and militarisation, the surveillance
apparatus from facial recognition to wide-area drones, bulk data and who buys
it, informants inside movements, stops and searches and the rights that answer
them, grand juries and compelled testimony, disarmament, the privacy tools that
turned out to be compromised, and the reforms and refusals set against all of it.

A crime report is not this subject. A robbery, a manhunt, a traffic collision
are crime stories; what the police did, were permitted to do, or were not held
to account for, is this wire. Every subject term carries the policing or
accountability words it must appear beside.

    python3 harvest_police.py
    python3 harvest_police.py --dry-run
    python3 harvest_police.py --fixtures DIR
"""
...
SOURCES_PATH = os.path.join(HERE, "sources_police.json")
OUT_PATH = os.path.join(HERE, "wire_police.json")
...
# --------------------------------------------------------------------------
# Subjects — after the Law Enforcement section: what the police are obliged to
# do, what they actually do, what they can see, and what you may refuse.
#
# Each subject is (term, context) pairs. The term must appear AND one of its
# context words. A crime report is not this subject; what the police did, were
# permitted to do, or were not held to account for, is.
# --------------------------------------------------------------------------
TOPICS = [
    ("duty", "The duty to protect", [
        ("duty to protect", []), ("no duty to protect", []), ("failure to protect", ["police", "officer", "law enforcement"]),
        ("special relationship doctrine", []), ("public duty doctrine", []),
        ("deshaney", []), ("castle rock", []), ("warren v", []),
        ("police liability", []), ("failure to intervene", []),
        ("911 call", ["ignored", "response", "failure", "delay", "no response"]),
        ("response time", ["police", "emergency", "criticised", "criticized"]),
        ("declined to intervene", []), ("stood by", ["police", "officers"]),
        ("failed to intervene", []), ("did not intervene", ["police", "officer"]),
        ("failed to act", ["police", "officer", "watchdog", "force"]),
    ]),
    ("immunity", "Immunity and accountability", [
        ("qualified immunity", []), ("sovereign immunity", ["police", "officer", "state"]),
        ("police accountability", []), ("civilian oversight", []), ("oversight board", ["police", "civilian"]),
        ("independent office for police conduct", []), ("iopc", []), ("consent decree", []),
        ("decertification", ["officer", "police"]), ("disciplinary action", ["officer", "police"]),
        ("internal affairs", []), ("misconduct", ["police", "officer", "law enforcement", "custody"]),
        ("cleared of wrongdoing", ["officer", "police"]), ("no charges", ["officer", "police", "shooting"]),
        ("settlement", ["police", "misconduct", "wrongful death", "brutality", "city paid"]),
        ("body camera", ["footage", "released", "withheld", "switched off", "policy"]),
        ("wall of silence", []), ("code of silence", []), ("blue wall", []),
        ("whistleblower", ["police", "officer", "retaliation"]),
        # forensic failure sits with accountability: it is how a wrong result survives review
        ("wrongful conviction", []), ("exoneration", []), ("evidence tampering", []),
        ("planted evidence", []), ("forensic", ["scandal", "failure", "discredited", "misconduct", "review"]),
        ("crime lab", ["scandal", "backlog", "error", "misconduct"]),
        ("expert testimony", ["discredited", "withdrawn", "unreliable"]),
    ]),
    ("corruption", "Corruption and extortion", [
        ("police corruption", []), ("police bribery", []), ("bribe", ["police", "officer", "checkpoint", "traffic"]),
        ("extortion", ["police", "officer", "checkpoint", "uniformed"]),
        ("shakedown", ["police", "officer"]), ("kickback", ["police", "officer", "commissioner"]),
        ("interpol", ["corruption", "bribe", "arrest", "abuse", "red notice"]),
        ("civil forfeiture", []), ("asset forfeiture", []), ("policing for profit", []),
        ("quota", ["ticket", "arrest", "citation", "stop"]),
    ...
    ("infiltration", "Criminal infiltration of the force", [
        ...
        ("leaked", ["police database", "police record", "investigation", "informant identity"]),
    ]),
    ("force", "Use of force", [
        ("police shooting", []), ("shot by police", []), ("killed by police", []), ("police killing", []),
        ("died in custody", []), ("custody death", []), ("death in custody", []),
        ("police brutality", []), ("excessive force", []), ("chokehold", []), ("neck restraint", []),
        ("taser", ["death", "deployed", "injury", "inquiry"]), ("baton round", []), ("rubber bullet", []),
        ("tear gas", []), ("kettling", []), ("no-knock raid", []), ("no knock warrant", []),
        ("stun grenade", []), ("less-lethal", []), ("less lethal", []),
        ("use of force", ["policy", "report", "review", "incident", "investigation"]),
    ]),
    ("militarisation", "Militarisation", [
        ("militarisation", ["police", "policing", "law enforcement"]),
        ("militarization", ["police", "policing", "law enforcement"]),
        ("1033 program", []), ("military surplus", ["police", "law enforcement"]),
        ("armoured vehicle", ["police", "deployed"]), ("armored vehicle", ["police", "deployed"]),
        ("mrap", []), ("swat", ["raid", "deployed", "team", "expansion"]),
        ("national guard", ["deployed", "protest", "policing", "city"]),
        ("troops", ["deployed", "streets", "policing", "domestic"]),
        ("state of emergency", ["police", "curfew", "protest", "security forces"]),
        ("crowd control weapon", []), ("riot police", []), ("gendarmerie", []),
        ("protest policing", []), ("protest crackdown", []),
    ]),
    ("surveillance", "Surveillance technology", [
        ("facial recognition", []), ("biometric surveillance", []), ("gait recognition", []),
        ("iris scan", ["police", "database", "checkpoint"]), ("ear biometric", []),
        ("thermal imaging", ["police", "surveillance", "camera", "warrant"]),
        ("remote fingerprint", []), ("automatic licence plate", []), ("automatic license plate", []),
        ("licence plate reader", []), ("license plate reader", []), ("alpr", []),
        ("stingray", ["device", "police", "surveillance", "cell"]), ("imsi catcher", []),
        ("cell-site simulator", []), ("cell site simulator", []),
        ("gorgon stare", []), ("wide-area surveillance", []), ("persistent surveillance", []),
        ("aerial surveillance", []), ("surveillance drone", []), ("police drone", []),
        ("drone", ["police", "surveillance", "deployed", "programme", "program", "swarm"]),
        ("smartdust", []), ("tracking device", ["police", "planted", "warrant", "covert"]),
        ("spyware", ["police", "phone", "target", "journalist", "activist", "deployed"]),
        ("pegasus", ["spyware", "phone", "target"]), ("predictive policing", []),
        ("gunshot detection", []), ("shotspotter", []), ("cctv", ["expansion", "network", "police", "recognition"]),
        ("laser microphone", []), ("through-wall", ["radar", "sensor", "imaging"]),
        # the algorithmic layer, alongside predictive policing above
        ("risk score", ["police", "offender", "algorithm", "assessment"]),
        ("gang database", []), ("watchlist", ["police", "added", "removed", "secret"]),
        ("social media monitoring", []), ("open-source intelligence", ["police", "unit"]),
        ("algorithmic", ["policing", "decision", "bias", "tool"]),
        ("ai report", ["police", "written", "generated"]), ("automated decision", ["police", "enforcement"]),
    ]),
    ("data", "Bulk data and the record", [
        ("mass surveillance", []), ("bulk collection", []), ("bulk data", []),
        ("nsa", ["collection", "surveillance", "programme", "program", "oversight", "senate"]),
        ("gchq", ["surveillance", "collection"]), ("warrantless", []),
        ("data broker", ["police", "law enforcement", "purchase", "location"]),
        ("location data", ["police", "purchased", "buys", "bought", "warrant", "sold",
                           "request", "broker", "trace", "tracking"]),
        ("buys", ["location data", "phone data", "surveillance", "spyware", "broker"]),
        ("geofence warrant", []), ("reverse warrant", []), ("keyword warrant", []),
        ("phone extraction", []), ("cellebrite", []), ("graykey", []),
        ("dna database", []), ("familial dna", []), ("biometric database", []),
        ("police database", ["misuse", "access", "breach", "abuse", "unauthorised"]),
        ("data retention", ["police", "law enforcement", "communications"]),
        ("interception", ["communications", "lawful", "warrant", "police"]),
        ("encryption", ["backdoor", "ban", "police access", "law enforcement", "lawful access"]),
        ("client-side scanning", []), ("chat control", []),
    ]),
    ("infiltrators", "Informants and undercover work", [
        ("undercover officer", []), ("undercover police", []), ("undercover unit", []),
        ("informant", ["police", "recruited", "paid", "handler", "movement", "activist"]),
        ("infiltrated", ["movement", "group", "protest", "activist", "organisation", "organization"]),
        ("agent provocateur", []), ("entrapment", []), ("spycops", []),
        ("undercover policing inquiry", []), ("confidential human source", []),
        ("sting operation", ["police", "fbi", "entrapment", "manufactured"]),
        ("infiltration", ["movement", "activist", "protest", "union", "campaign"]),
    ]),
    ("rights", "Stops, searches and what you may refuse", [
        ("stop and search", []), ("stop and frisk", []), ("stop-and-frisk", []),
        ("search warrant", []), ("warrantless search", []), ("consent search", []),
        ("fourth amendment", []), ("fifth amendment", []), ("right to remain silent", []),
        ("right to counsel", []), ("miranda", ["rights", "warning", "waiver"]),
        ("know your rights", []), ("if an agent knocks", []),
        ("racial profiling", []), ("checkpoint", ["stop", "search", "papers", "police"]),
        ("papers please", []), ("identity check", ["police", "required", "refuse"]),
        ("recording police", []), ("right to record", []), ("filming police", []),
    ]),
    ("compelled", "Grand juries and compelled testimony", [
        ("grand jury", []), ("subpoena", ["testify", "quash", "refuse", "witness", "resist"]),
        ("civil contempt", []), ("contempt of court", ["jailed", "refuse", "testify"]),
        ("compelled testimony", []), ("immunity", ["grand jury", "testimony", "prosecutor", "granted"]),
        ("jailed for refusing", []), ("refused to testify", []),
        ("gag order", []), ("national security letter", []),
    ]),
    ("disarm", "Disarmament and pacification", [
        ("gun control", []), ("firearms law", []), ("weapons ban", []), ("disarmament", ["civilian", "population"]),
        ("licensing", ["firearm", "gun", "weapon"]), ("confiscation", ["firearm", "gun", "weapon"]),
        ("red flag law", []), ("self-defence law", []), ("self-defense law", []),
        ("stand your ground", []), ("right to bear arms", []),
        ("nonviolence", ["doctrine", "imposed", "training", "ideology"]),
    ]),
    ("privacy", "Privacy tools and their compromise", [
        ("vpn", ["logging", "logs", "breach", "no-log", "leak", "seized", "exposed"]),
        ("tor", ["network", "funding", "deanonymis", "deanonymiz", "compromised", "relay"]),
        ("no-log policy", []), ("encrypted messaging", ["ban", "backdoor", "police", "access", "seized"]),
        ("signal", ["ban", "backdoor", "subpoena", "encryption"]),
        ("device seizure", []), ("phone search", ["border", "police", "warrant", "unlock"]),
        ("compelled decryption", []), ("password compelled", []),
        ("side-channel", []), ("firmware", ["backdoor", "leak", "implant"]),
        ("supply chain implant", []),
    ]),
    ("reform", "Oversight, reform and refusal", [
        ("police reform", []), ("defund the police", []), ("abolition", ["police", "prison"]),
        ("community safety", ["alternative", "programme", "program", "unarmed"]),
        ("unarmed response", []), ("crisis response team", []), ("violence interruption", []),
        ("police union", ["contract", "blocked", "opposed", "influence"]),
        ("officer resigns", ["protest", "refusal", "conscience"]),
        ("refused an order", ["officer", "police"]),
        ("public inquiry", ["police", "policing", "misconduct"]),
        ("royal commission", ["police", "policing"]),
        ("truth commission", ["police", "security forces"]),
        ("reparations", ["police", "victims", "brutality"]),
    ]),

    ("custody", "Police custody, before charge", [
        # Scoped to the police's own hands: arrest to charge or release. Prisons,
        # sentencing, parole and corrections belong to the courts wire.
        ("police custody", []), ("police cell", []), ("holding cell", ["police", "station"]),
        ("police station", ["death", "held", "detained", "assault", "cell"]),
        ("pre-charge detention", []), ("detention without charge", []),
        ("incommunicado", []), ("held without charge", []), ("arbitrary detention", []),
        ("arrest quota", []), ("mass arrest", []), ("dragnet arrest", []),
        ("access to a lawyer", ["denied", "refused", "delayed", "detained"]),
        ("right to a lawyer", ["denied", "refused", "custody"]),
        ("juvenile", ["detained", "custody", "interrogated", "police"]),
        ("welfare check", ["died", "custody", "cell"]),
        ("remand", ["police", "pre-trial", "overcrowd"]),
    ]),
    ("torture", "Torture and coerced confession", [
        ("torture", ["police", "custody", "detainee", "interrogation", "allegation", "confession"]),
        ("ill-treatment", ["custody", "police", "detainee"]),
        ("coerced confession", []), ("forced confession", []), ("confession under duress", []),
        ("beaten in custody", []), ("waterboarding", []), ("stress position", []),
        ("interrogation technique", []), ("enhanced interrogation", []),
        ("special rapporteur on torture", []), ("committee against torture", []),
        ("cpt report", []), ("national preventive mechanism", []),
        ("optional protocol", ["torture", "cat"]),
    ]),
    ("disappearance", "Enforced disappearance and death squads", [
        ("enforced disappearance", []), ("forcibly disappeared", []), ("disappeared", ["police", "detained", "custody", "activist", "families"]),
        ("death squad", []), ("extrajudicial killing", []), ("extrajudicial execution", []),
        ("summary execution", []), ("mass grave", ["police", "security forces", "investigation"]),
        ("drug war", ["killings", "deaths", "operation", "campaign"]),
        ("shoot to kill", []), ("encounter killing", []), ("fake encounter", []),
        ("clandestine grave", []), ("clandestine graves", []), ("off-the-books", ["unit", "detention", "site"]),
        ("the disappeared", []), ("mothers of the disappeared", []), ("missing persons", ["forced", "state", "security forces", "graves"]),
        ("exhumed", ["clandestine", "mass grave", "victims", "disappeared", "forensic team"]),
        ("exhumation", ["disappeared", "mass grave", "victims", "clandestine"]),
        ("black site", []), ("secret detention", []),
    ]),
    ("border", "Border and immigration enforcement", [
        ("border force", []), ("border patrol", []), ("frontex", []),
        ("pushback", ["border", "migrants", "coast", "asylum"]),
        ("deportation", ["raid", "flight", "sweep", "detention", "expedited"]),
        ("immigration raid", []), ("immigration detention", []), ("detention centre", ["immigration", "migrant"]),
        ("detention center", ["immigration", "migrant"]),
        ("ice", ["raid", "detention", "deportation", "agents", "enforcement"]),
        ("coast guard", ["interdiction", "pushback", "migrants", "boat"]),
        ("asylum", ["detained", "refused entry", "border", "pushback"]),
        ("biometric border", []), ("eurodac", []), ("entry-exit system", []),
        ("visa database", []), ("carrier sanctions", []), ("externalisation", ["border", "migration"]),
    ]),
    ("transnational", "Transnational repression", [
        ("transnational repression", []), ("red notice", ["abuse", "misuse", "interpol", "politically"]),
        ("rendition", []), ("extraordinary rendition", []),
        ("forced return", []), ("refoulement", []),
        ("exile", ["targeted", "surveilled", "harassed", "threatened", "assassination"]),
        ("diaspora", ["surveillance", "intimidation", "harassment", "threats"]),
        ("assassination", ["abroad", "exile", "dissident", "journalist", "plot"]),
        ("embassy", ["surveillance", "harassment", "abduction", "intimidation"]),
        ("mutual legal assistance", ["abuse", "politically motivated"]),
        ("extradition", ["politically motivated", "refused", "abuse", "dissident"]),
    ]),
    ("private", "Private and parallel policing", [
        ("private security", ["guards", "contractor", "force", "shot", "evicted", "protest"]),
        ("security contractor", []), ("mine security", []), ("plantation guard", []),
        ("company security", ["land", "protest", "defender", "eviction"]),
        ("vigilante", []), ("militia", ["state-backed", "police", "auxiliary", "armed"]),
        ("paramilitary", ["police", "auxiliary", "state-backed", "deployed"]),
        ("land defender", ["killed", "attacked", "threatened", "criminalised"]),
        ("environmental defender", ["killed", "attacked", "threatened", "criminalised"]),
        ("eviction", ["force", "security", "police", "violent", "demolition"]),
        ("auxiliary police", []), ("civil defence force", []),
    ]),
    ("political", "Counter-terrorism as political policing", [
        ("terrorism designation", []), ("terrorist organisation", ["designated", "listed", "banned", "activist"]),
        ("counter-terrorism", ["powers", "law", "used against", "activists", "protest"]),
        ("prevent", ["strategy", "duty", "referrals"]),
        ("preventive detention", []), ("administrative detention", []),
        ("state of exception", []), ("emergency powers", ["police", "protest", "extended"]),
        ("foreign agent", ["law", "registered", "designated", "ngo"]),
        ("protest ban", []), ("banned demonstration", []), ("proscribed", ["organisation", "group"]),
        ("sedition", []), ("treason charge", ["activist", "journalist", "protest"]),
        ("anti-protest law", []), ("public order act", []), ("gag law", []),
        ("criminalisation", ["protest", "solidarity", "activism", "dissent"]),
        ("criminalization", ["protest", "solidarity", "activism", "dissent"]),
    ]),
    ("targets", "Who gets policed", [
        ("racial profiling", []), ("ethnic profiling", []), ("religious profiling", []),
        ("roma", ["police", "raid", "eviction", "profiling", "discrimination"]),
        ("indigenous", ["police", "raid", "custody", "deaths", "policing", "arrest"]),
        ("migrant", ["police", "raid", "stop", "checks", "arrest"]),
        ("sex worker", ["police", "raid", "arrest", "extortion", "violence"]),
        ("drug user", ["police", "arrest", "raid", "crackdown"]),
        ("homeless", ["police", "arrest", "sweep", "clearance", "fine", "encampment"]),
        ("lgbt", ["police", "raid", "arrest", "crackdown", "entrapment"]),
    ...
    ("export", "Training and equipment export", [
        ...
        ("security assistance", []), ("police assistance", ["foreign", "programme", "programme"]),
        ("crowd control", ["export", "sold", "supplied", "shipment", "licence"]),
        ("tear gas", ["export", "sold", "supplied", "shipment", "manufacturer"]),
        ("surveillance export", []), ("spyware export", []),
        ("export licence", ["surveillance", "security", "police", "crowd control"]),
        ("export license", ["surveillance", "security", "police", "crowd control"]),
        ("arms fair", ["police", "security", "surveillance"]),
        ("technical assistance", ["police", "security forces", "training"]),
    ]),
    ("sexual", "Sexual violence by police", [
        ("custodial rape", []), ("sexual assault", ["officer", "police", "custody", "detainee"]),
        ("sexual misconduct", ["officer", "police", "chief", "force"]),
        ("sexual exploitation", ["informant", "police", "officer", "vulnerable"]),
...
ANCHOR = [
    # Control-and-accountability language. A crime report is not this subject;
    # what the police did, were permitted to do, or escaped answering for, is.
    "qualified immunity", "police accountability", "civilian oversight", "consent decree",
    "police watchdog", "police misconduct", "police corruption", "police brutality",
    "excessive force", "police shooting", "killed by police", "died in custody",
    "custody death", "death in custody", "extrajudicial killing", "police impunity",
    "no-knock", "stop and search", "stop and frisk", "racial profiling",
    "police militarisation", "police militarization", "riot police", "protest policing",
    "facial recognition", "predictive policing", "mass surveillance", "bulk collection",
    "warrantless", "geofence warrant", "cell-site simulator", "imsi catcher", "stingray",
    "licence plate reader", "license plate reader", "surveillance drone", "police drone",
    "spyware", "phone extraction", "cellebrite", "compelled decryption",
    "undercover police", "spycops", "agent provocateur", "entrapment", "police informant",
    "grand jury", "civil contempt", "compelled testimony", "national security letter",
    "know your rights", "right to record", "search warrant", "warrantless search",
    "duty to protect", "failure to intervene", "code of silence", "blue wall",
    "civil forfeiture", "asset forfeiture", "policing for profit",
    "police reform", "defund the police", "decertification", "police union contract",
    "body camera", "internal affairs", "inquest", "police inquiry",
]

BLOCK = [
    # the daily blotter. A robbery, a manhunt, a traffic collision — these are
    # crime stories, not stories about policing as a system.
    "police are searching for", "police appeal for", "man charged with", "woman charged with",
    "arrested on suspicion of theft", "burglary suspect", "shoplifting", "missing person",
    "traffic collision", "road closed", "crash investigation", "drink driving",
    "police warn of scam", "fraud warning", "wanted poster", "crime stoppers",
    "police say the victim", "homicide investigation continues",
    # entertainment and metaphor
    # the courts and corrections wire covers what happens after charge
    "sentenced to", "sentencing hearing", "parole board", "parole hearing", "prison sentence",
    "jury selection", "trial begins", "trial continues", "found guilty", "pleaded guilty",
    "appeal court upheld the conviction", "prison overcrowding", "prison riot", "prison escape",
    "released on parole", "life sentence", "death row", "execution date",
    "police drama", "crime drama", "police procedural", "cop show", "film review",
    "fashion police", "grammar police", "police academy film", "video game",
]

DECIDED = [
    "approved", "signed", "awarded", "granted", "ratified", "enacted", "passed",
    "ruling", "ruled", "struck down", "upheld", "overturned", "judgment", "judgement",
    "took effect", "came into force", "repealed", "revoked", "cancelled", "terminated",
    "nationalised", "nationalized", "expropriated", "seized", "auctioned", "sold",
    "defaulted", "restructured", "written off", "disbursed", "suspended",
]
INSTITUTIONAL = [
    "international monetary fund", "world bank", "world trade organization", "united nations",
    "unctad", "oecd", "bank for international settlements", "financial stability board",
    "central bank", "finance ministry", "treasury", "ministry of finance", "auditor general",
    "official gazette", "court filing", "arbitration award", "parliamentary committee",
    "peer-reviewed", "published in", "dataset", "aiddata", "tax justice network",
    "official data", "government figures", "national statistics",
]
MEASURED = [
    "per cent", "percent", "%", "billion", "million", "trillion", "basis points",
    "share of gdp", "per capita", "hectares", "square kilometres", "tonnes",
    "figures show", "fell by", "rose by", "increase of", "decrease of", "estimated",
    "median", "average", "ranked", "index",
]
PENDING = [
    "proposed", "draft law", "under review", "consultation", "tender", "bidding round",
    "expected to", "due to decide", "talks", "negotiation", "in negotiations",
    "deadline", "next month", "next year", "review scheduled", "pending approval",
]
...
LOCAL_TERMS = {
    "surveillance": [
        # The commercial spyware cluster arrived weekly and matched nothing:
        # the subject was written for state surveillance programmes and not for
        # the vendors, the alerts or the victims.
        ("spyware", None), ("mercenary surveillance", None), ("threat notification*", None),
        ("pegasus", None), ("predator spyware", None), ("commercial surveillance", None),
        ("stalkerware", None), ("phone hacking", ["state", "police", "journalist*", "activist*"]),
        ("logiciel espion", None), ("programa espía", None), ("spionagesoftware", None),
        ("шпионск", ["программ", "по"]), ("间谍软件", None), ("スパイウェア", None),
    ],
    "custody": [
        ("inquest", None), ("death in custody", None), ("died in custody", None),
        ("reopened inquiry", None), ("reopened inquest", None),
        ("steve biko", None), ("custodial death*", None),
    ],
}
...
FILL_TERMS = {
    "disarm": [
        ("debate sobre el", None), ("debate sobre o", None),
        ("debatte bewaffnung der", None), ("débat sur l'armement", None),
        ("police non armée", None), ("policía sin armas", None),
        ("polícia sem armas", None), ("unbewaffnete polizei modell", None),
        ("テーザー 導入 検証", None), ("电击枪 配备 检讨", None),
        ("警察 不配枪 模式", None), ("警察官 拳銃 携帯", None),
        ("경찰 비무장 모델", None), ("테이저건 도입 검토", None),
    ],
    "duty": [
        ("amtshaftung polizei urteil", None), ("ação por omissão", None),
        ("demanda por omisión", None), ("klage wegen untätigkeit", None),
        ("omissione di soccorso", None), ("plainte pour carence", None),
        ("responsabilidad de la", None), ("responsabilidade da polícia", None),
        ("responsabilità della polizia", None), ("responsabilité de la", None),
        ("бездействие полиции иск", None), ("ответственность полиции решение", None),
        ("دعوى تقصير الشرطة", None), ("مسؤولية الشرطة حكم", None),
        ("警察 不作為 訴訟", None), ("警察 责任 判决", None),
        ("警察の責任 判決", None), ("警方 不作为 诉讼", None),
        ("경찰 부작위 소송", None), ("경찰 책임 판결", None),
    ],
    "export": [
        ("ausfuhrgenehmigung überwachungstechnik", None), ("divieto di export", None),
        ("exportverbot polizeiausrüstung", None), ("interdiction d'exporter du", None),
        ("licence d'exportation de", None), ("licencia de exportación", None),
        ("licenza di esportazione", None), ("licença de exportação", None),
        ("prohibición de exportar", None), ("proibição de exportar", None),
        ("запрет экспорта спецсредств", None), ("экспортная лицензия на", None),
        ("治安機材 輸出規制", None), ("监控技术 出口许可", None),
        ("監視技術 輸出許可", None), ("警用装备 出口管制", None),
        ("감시 기술 수출", None), ("진압 장비 수출", None),
    ],
    "privacy": [
        ("gesetz hintertür verschlüsselte", None), ("journaux vpn saisis", None),
        ("legge backdoor messaggistica", None), ("lei de backdoor", None),
        ("ley de puerta", None), ("log vpn sequestrati", None),
        ("loi sur les", None), ("registos de vpn", None),
        ("registros de vpn", None), ("vpn ログ 押収", None),
        ("vpn 日志 被查封", None), ("vpn 기록 압수", None),
        ("vpn-protokolle beschlagnahmt", None), ("закон о бэкдоре", None),
        ("изъятие логов vpn", None), ("قانون الباب الخلفي", None),
        ("مصادرة سجلات vpn", None), ("加密通讯 后门 立法", None),
        ("暗号化メッセージ バックドア 法案", None), ("암호화 메신저 백도어", None),
    ],
}
...
def run(dry_run=False, fixtures=None):
    ...
    if dry_run:
        print("\n--dry-run: wire_police.json not written")
        return payload
