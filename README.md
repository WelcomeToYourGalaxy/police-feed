# The police wire — law enforcement, worldwide
...
`harvest_police.py` runs every two hours in GitHub Actions, reads 61 wires, tags and weighs what
it finds, and writes `wire_police.json`. `index.html` loads that file and renders it.
...
the argument. The balance of the feed is the balance of `sources_police.json`, which is meant to
be edited.
...
| `harvest_police.py` | `/harvest_police.py` | The harvester. Self-contained. |
| `sources_police.json` | `/sources_police.json` | The wire list, with each wire's standing. |
| `wire_police.json` | `/wire_police.json` | The output the page reads. Empty placeholder until the first run. Never hand-edit. |
| `police-feed-weebly-embed.html` | `/police-feed-weebly-embed.html` | The page wrapped for a Weebly Embed Code element. Regenerate after changing `index.html`. |
...
3. Actions tab → **Harvest the police wire** → *Run workflow*.
4. Settings → Pages → **Deploy from a branch**, branch `main`, folder `/ (root)`.
5. Confirm
   `https://raw.githubusercontent.com/WelcomeToYourGalaxy/police-feed/main/wire_police.json`
   loads in a browser.

If the repository is named something other than `police-feed`, change `REPO` near the top of the
...
```bash
python3 harvest_police.py              # full run
python3 harvest_police.py --dry-run    # harvest and report, write nothing
python3 harvest_police.py --fixtures tests/
```
